...
function applyRoleUI(){
  ...
  const a = document.querySelector('a[data-view="produtos"]');
  const li = a ? a.closest("li") : null;
  if (li) li.style.display = isAdmin ? "" : "none";
}

function showView(name){
  ["view-dashboard","tab-pdv","tab-historico","view-produtos"].forEach(id=>{
    const el = $("#"+id);
    if (!el) return;
  ...
  const map = {
    ...
    vendas:    "#tab-pdv",       // PDV visível; histórico abre em overlay
    historico: "#tab-historico", // mantido (não usado diretamente)
    produtos:  "#view-produtos",
  };
  const sel = map[name]; if (!sel) return;
  const node = $(sel); if (node){ node.classList.remove("is-hidden"); node.setAttribute("aria-hidden","false"); }
}
...
/** Cria o overlay no DOM caso não exista (não precisa editar o HTML) */
